  ...
  if (field.type === "date") {
    ...
    return (
      <View style={{ gap: 6 }}>
        <TextInput
          value={value}
          onChangeText={(nextValue) => onChange(normalizeDateInputValue(nextValue))}
          placeholder={field.placeholder ?? "AAAA-MM-DD o DD/MM/AAAA"}
        ...
        <Text style={{ color: preview ? "#1e3a5f" : "#64748b", fontSize: 12 }}>
          {preview ? `Fecha seleccionada: ${preview}` : "Formato sugerido: AAAA-MM-DD o DD/MM/AAAA."}
        </Text>
      </View>
    );
  }
    ...
    return (
      <View style={{ gap: 8 }}>
        {field.options?.map((option) => {
          const active = option.option === value;

          return (
            <Pressable
              key={option.option}
              onPress={() => onChange(option.option)}
              style={({ hovered, pressed }) => ({
                borderWidth: 1,
                borderColor: active ? "#1d4ed8" : "#cbd5e1",
                backgroundColor: active ? "#eff6ff" : pressed ? "#f8fbff" : hovered ? "#f8fbff" : "white",
                borderRadius: 12,
                paddingVertical: 10,
                paddingHorizontal: 12
              })}
            >
              <Text style={{ color: "#0f172a" }}>{option.option}</Text>
            </Pressable>
          );
        })}
      </View>
...
export default function SolicitudPorCodigoScreen() {
  ...
  const visibleRequestTypes = useMemo(() => {
    ...
    const filteredTypes = catalog.requestTypes.filter(
      (type) => !restrictedTypeCodes.has(type.code) || Boolean(currentUser?.canManagePeopleFlows || currentUser?.isAdmin)
    );
  ...
  const routeSteps = useMemo(() => {
    ...
    const withoutManager = workflowSteps.filter((step) => step.code !== "IMMEDIATE_LEAD");
        ...
        <View style={{ flexDirection: isWide ? "row" : "column", alignItems: "flex-start", gap: 12 }}>
          ...
          <View style={{ backgroundColor: "white", borderRadius: 30, borderWidth: 1, borderColor: "#bfd2e7", padding: 16, gap: 10 }}>
            ...
            <View style={{ gap: 6 }}>
              ...
              >
                <Text style={{ color: department ? "#0f2440" : "#64748b" }}>
                  {department || "Selecciona un departamento de la lista"}
                </Text>
              </View>
              {(catalog?.departments?.length ?? 0) > 0 ? (
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
                  {(catalog?.departments ?? []).map((item) => (
                    <Pressable
                      ...
                      style={({ hovered, pressed }) => ({
                        borderWidth: 1,
                        borderColor: department === item ? "#0b5ed7" : "#cbd5e1",
                        borderRadius: 999,
                        paddingHorizontal: 10,
                        paddingVertical: 4,
                        backgroundColor:
                          department === item ? "#eff6ff" : pressed ? "#edf3ff" : hovered ? "#edf3ff" : "#f8fafc"
                      })}
                    >
